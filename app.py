...
# --- OCR Extraction with fuzzy spatial matching ---
def extract_forms_from_ocr(response):
    ...
    try:
        ...
        # Find block just below a label
        def find_below(label_hint, blocks, x_tol=80, y_min=5, y_max=150):
            label_block = match_label(label_hint, blocks)
            if not label_block:
                return ""
        ...
        for idx in range(1, 4):
            blocks = forms[idx]
            fields = {label: find_below(label, blocks) for label in known_labels}
            table = [b["text"] for b in blocks if len(b["text"].split()) >= 2][:11]
            fields["TABLE_ROWS"] = table
...
st.title("📄 Greek OCR Form Parser")

uploaded_file = st.file_uploader("📎 Upload a scanned Greek form", type=["png", "jpg", "jpeg"])
...
if uploaded_file:
    ...
    for idx, form in enumerate(forms, start=1):
        with st.expander(f"📄 Φόρμα {idx}", expanded=(idx == 1)):
            ...
            r1 = st.columns(5)
            r1[0].text_input("ΑΡΙΘΜΟΣ ΜΕΡΙΔΟΣ", form.get("ΑΡΙΘΜΟΣ ΜΕΡΙΔΟΣ", ""), key=f"{idx}_0")
            r1[1].text_input("ΕΠΩΝΥΜΟ", form.get("ΕΠΩΝΥΜΟ", ""), key=f"{idx}_1")
            r1[2].text_input("ΚΥΡΙΟΝ ΟΝΟΜΑ", form.get("ΚΥΡΙΟΝ ΟΝΟΜΑ", ""), key=f"{idx}_2")
            r1[3].text_input("ΟΝΟΜΑ ΠΑΤΡΟΣ", form.get("ΟΝΟΜΑ ΠΑΤΡΟΣ", ""), key=f"{idx}_3")
            r1[4].text_input("ΟΝΟΜΑ ΜΗΤΡΟΣ", form.get("ΟΝΟΜΑ ΜΗΤΡΟΣ", ""), key=f"{idx}_4")

            r2 = st.columns(3)
            r2[0].text_input("ΤΟΠΟΣ ΓΕΝΝΗΣΕΩΣ", form.get("ΤΟΠΟΣ ΓΕΝΝΗΣΕΩΣ", ""), key=f"{idx}_5")
            r2[1].text_input("ΕΤΟΣ ΓΕΝΝΗΣΕΩΣ", form.get("ΕΤΟΣ ΓΕΝΝΗΣΕΩΣ", ""), key=f"{idx}_6")
            r2[2].text_input("ΚΑΤΟΙΚΙΑ", form.get("ΚΑΤΟΙΚΙΑ", ""), key=f"{idx}_7")
